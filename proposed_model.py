...
def build_dbn_bilstm_regressor(input_shape):
    ...
    x = Bidirectional(LSTM(64, return_sequences=True))(seq)
    # cbam_a = CBAMBLOCK()(x)
    ...
    x = Bidirectional(LSTM(64, return_sequences=False))(mca_out)

    out = Dense(32, activation="relu")(x)
    out = Dropout(0.1)(out)
    out = Dense(16, activation="relu")(out)
    output = Dense(1, activation="linear")(out)
...
def train_three_regressors(x_train1, y_train1,
    ...
    dbn2, model2 = build_dbn_bilstm_regressor((timesteps1, features1), )

    cprint("Building DBN+BiLSTM regressor for dataset 3 (Wind)", color="white", on_color="on_magenta")
    dbn3, model3 = build_dbn_bilstm_regressor((timesteps2, features2), )
