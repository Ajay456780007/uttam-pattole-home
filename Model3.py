...
def build_rnn_regressor(input_shape):
    model = Sequential([
...
def train_three_regressors(x_train1, y_train1,
    ...
    model1 = build_rnn_regressor(x_train1.shape[1:])
    model2 = build_rnn_regressor(x_train2.shape[1:])
    model3 = build_rnn_regressor(x_train3.shape[1:])
...
def RNN_1(x_train1, x_test1, y_train1, y_test1, x_train2, x_test2, y_train2, y_test2, x_train3, x_test3, y_train3,
          y_test3, C_train, C_test, epochs):
    # 1. Train three RNN regressors
